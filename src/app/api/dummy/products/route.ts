...
import { Product, ProductsResponse, SearchFilters } from '@/types/product';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sort = searchParams.get('sort') as 'price' | 'rating' | 'title' || undefined;
    ...
    // Generar ETag simple
    const etag = `"${Buffer.from(JSON.stringify(transformedData)).toString('base64').slice(0, 16)}"`;
