...
export function PriceChart({ currentPrice, productName, className }: PriceChartProps) {
  ...
  const generatePriceHistory = (): PriceData[] => {
    ...
    const seed = currentPrice.toString().split('').reduce((a, b) => a + b.charCodeAt(0), 0);
  ...
  const getTrendIcon = () => {
    if (priceChangePercent > 2) return <TrendingUp className="w-4 h-4 text-green-600" />;
    if (priceChangePercent < -2) return <TrendingDown className="w-4 h-4 text-red-600" />;
    ...
    <Card className={className}>
      ...
      <CardContent>
        ...
        <div className="sr-only">
          <p>
            Gráfica de historial de precios para {productName}. 
            El precio actual es {formatPrice(currentPrice)}. 
            En los últimos 12 meses, el precio ha {priceChangePercent > 0 ? 'aumentado' : 'disminuido'} 
